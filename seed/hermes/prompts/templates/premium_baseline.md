...
1. **Module order is binding.** The left-to-right sequence of modules in the brief is an absolute constraint, not a preference. Do not swap, skip, insert, or rearrange any module.
...
4. **Adjacent modules are adjacent.** If two modules are listed as directly adjacent, there is zero space, panel, filler strip, or cabinet between them. Direct contact, no gap.
5. **Do not extend any element beyond its source boundary.** Islands, runs, and walls end where the source shows they end.
6. **Render what is drawn.** Do not add anything not shown in the source image — no plants, no countertop objects, no extra shelving, no decorative lighting, no people.
