...
**HARD conflicts (block-worthy) for sketch-to-render** — only these five, nothing else:
...
5. A door, entry, opening, window, threshold, or arch is moved to a different wall/side, shifted to the wrong side of the wall, or rendered with the wrong visible swing/handing.
...
- hard_conflicts: object/element ADDED that is not in the source, object REMOVED from source, count changed, LEFT-TO-RIGHT ELEMENT ORDER changed, directly-adjacent elements have a gap or extra element between them in the output, door/opening wall or swing/handing changed, label or text wrong.
