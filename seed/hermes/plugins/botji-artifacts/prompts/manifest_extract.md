...
- `fidelity_requirements` must be pre-built review strings ready for `artifact_review(fidelity_requirements=[...])`. Example: "Total element count: exactly 7 units", "Left wall (L→R): REF tower — 4-drawer base — oven stack", "REF tower is directly adjacent to 4-drawer base on left wall — zero gap".
